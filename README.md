...
- [~] **Phase 3 — DLSS Super Resolution.** *DLSS is now device-ready:* device created with DLSS's required VK extensions + 1.2 features, `slSetVulkanInfo` device hand-off, NGX context initialized (GUID `projectId` required), and `slDLSSGetOptimalSettings` returns real per-preset render resolutions (Quality 569×320, Performance 427×240 … for 854×480). *Remaining: low-res world pass, tag color/depth/MV/output + `slSetConstants` + `slEvaluateFeature`, composite, HUD on top.*
